...
> Cathedral II code graph, dream synthesis, skillify, claw-test) fully
> subsume our `skills/kos-jarvis/` extension pack? See **P0 — next
> session** below.
>
...
### [ ] gbrain vs kos-jarvis extension-pack consolidation review

**Why**: v0.25.0 brings dream synthesis, eval capture/replay, Cathedral II
code edges, skillify production-grade scaffolding, and `claw-test` E2E
harness. Many of these overlap with what we built in
`skills/kos-jarvis/` (dikw-compile, evidence-gate, confidence-score,
kos-lint, kos-patrol, digest-to-memory). Maintaining two systems
long-term is fragile. Lucien explicitly asked for a holistic review
before next sync.

**What**:
1. Inventory: list every `skills/kos-jarvis/*` skill + its callers.
2. Map each to a v0.25.0 upstream feature (covered ✓ / partial / unique).
3. Classify the unique-to-fork items: keep, merge upstream-PR, deprecate.
4. Pilot test: pick 1-2 fork skills with strong upstream overlap (e.g.
   `kos-lint` ↔ `gbrain doctor` + `gbrain frontmatter`), retire to
   measure regression.
5. Write a deprecation plan with timeline (e.g. "v0.26.0 sync deprecates
   X, v0.27.0 removes Y").

**Output**: `docs/KOS-JARVIS-CONSOLIDATION-PLAN.md` with the matrix +
deprecation steps + risk register.
