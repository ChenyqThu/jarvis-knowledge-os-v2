...
2. Read [`docs/JARVIS-ARCHITECTURE.md`](docs/JARVIS-ARCHITECTURE.md) — the
   ...
   operates ↔ OpenClaw executes). Latest sync story: **§6.43 v0.42.63.0
   upstream sync (2026-07-21, **106 commits** / v0.42.59.0 → v0.42.63.0 / 443
   files — largest batch to date, yet **zero-conflict merge**. `merge-tree`
   flagged 8 "changed in both" files incl. 3 rewrite-scale hits on fork src
   patches (gateway.ts +378/−186, pglite-engine.ts +203/−38, extract-atoms.ts
   +112/−82); all auto-resolved by the recursive virtual base — **`changed in
   both` is a verify-list, NOT a conflict forecast**. Schema **v122 → v124, two
   migrations applied** (v123 configurable FTS language, no backfill at our
   `english` default; v124 drops compiled_truth from pages.search_vector, #2704
   tsvector 1MB overflow). **§6.39's multi-statement-DDL P0 still did not
   trigger — this time genuinely verified**: both migrations are `sql: ''` +
   handler with per-statement executeRaw, structurally unable to reach the
   `conn.unsafe` batch path. **P0 stays OPEN and unproven** — needs a
   multi-statement `sql:` migration to settle. Fork territory zero-invasion,
   5 fork src/ patches survived (gateway.ts embed-retry block stayed *inside*
   upstream's `__embedInputTypeStore` context — §6.35 composition intact).
   Prod 27,469 pages / 71,415 chunks / 0 NULL / te3@1536 (6 cosmetic mislabels,
   was 48); brain_score 81/100 (+1), FAILs 3→2 as orphan_ratio fell 92%→62%;
   doctor embedding_provider ✓ **724ms — 16× faster than §6.40's 11,848ms via
   the avman relay**, quantifying §6.41's direct-connect win; both /health →
   ...
   Previous: §6.40 v0.42.59.0 sync (2026-07-13, 7 commits, zero-migration
